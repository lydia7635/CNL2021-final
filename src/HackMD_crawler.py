...
def get_updated_time(time_html):
    ...
    if "月" in updated_time[0]:
        ...
        time_tuple = dt.datetime(int(year), int(month), int(day), int(hour), int(minute)+1, 0).timetuple()
    else:
...
def get_summary(url, content, keyword):
    summary = None
...
def get_hackmd_update(url, keywords, last_update_time):
    ...
    for keyword in keywords:
        if keyword == None:
            updates += [ {"url": url, "published_time": updated_time, "status": "success", "title": title} ]
        else:
            content = driver.find_element_by_class_name("ui-view-area").text
            summary = get_summary(url, content, keyword)
            if summary != None:
                updates += [ {"url": url, "keyword": keyword, "published_time": updated_time, "status": "success", "content": summary, "title": title} ]
